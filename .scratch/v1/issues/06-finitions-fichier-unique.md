...
- [x] Le fichier unique fonctionne intégralement (génération, 3 modes, son) avec le réseau coupé, ouvert en double-clic depuis n'importe quel dossier.
- [x] Aucune requête réseau au chargement (vérifié dans l'onglet Réseau).
- [x] En-tête « Exercice nᵒ / Leçon » cohérent avec la configuration choisie.
- [x] La version dossier (développement) et le fichier unique restent générés depuis la même source, avec une commande de build documentée dans le README.
